...
\textcolor{red}{これまでの原稿ではどんな解析をしていたのかがよく分からない．（$r^2$は線形回帰から得られた決定係数$R^2$の間違い？あるいは，相関係数$r$の間違い？）$R^2$が正しい場合，$R^2$は非常に小さい値であるため，親潮の位置（x軸．説明変数）は0歳魚尾数（y軸．応答変数）をほとんど説明できていない．また相関関係を見ていた場合であっても，親潮と0歳魚尾数に関係性があると言えるほど相関係数は大きくない．いずれにせよ，解析方法をきちんと記述せずに考察に結果を入れ込むのは不適切なので削除しました．}
